...
>> Modifiquei o gráfico abaixo para o violin plot pois ele permite observar também o a distribuição dos dados, coisa que o line, neste caso, oculta; mas como tudo nessa revisão, é apenas uma sugestão.
...
>>  O gráfico abaixo é uma opção ao anterior, que separa por temporadas e mantém cada série em uma linha, para evitar a sobreposição da informação e facilitar a comparação, usando ainda linhas e pontos.

```{r}
plot_serie <- function(name) {
    plot <- sumarios3 %>%
        filter(series_name == name) %>%
        ggplot(aes(x = season_ep, y =user_votes , color = season)) + 
        geom_line() + 
        geom_point() + 
...
p_ahs <- plot_serie("American Horror Story")
p_bm <- plot_serie("Black Mirror")
p_pd <- plot_serie("Penny Dreadful")
p_s <- plot_serie("Sherlock")

cowplot::plot_grid(p_ahs, p_s, p_bm, p_pd, ncol = 1)
```
...
>> A mesma estratégia pode ser usada aqui, ou podemos pensar em um outras visualizações que transmitam as informações desejadas.
...
cv = series %>% 
    ...
    summarise(media_avaliacao = mean(user_rating),
              ...
              cv = media_avaliacao/dp_avaliacao)

knitr::kable(cv)
```

>> Pode-se usar a visão tabular ou ainda um gráfico, como segue:

```{r}
library(reshape2)
cv.long <- melt(cv, id.vars = c("series_name"))
cv.long %>%
    ggplot(aes(x = series_name, y = value, fill = variable)) +
